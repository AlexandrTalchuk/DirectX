...
void GenerateHMap()
{
    ...
    for (; i < (u / 4 - 1); i++)
    {
        for (int j = 0; j < v; j++)
        {
            ...
            float h = xx * yy * xx * yy * -5 + 0.1f / (0.5f + (x * x + y * y * 2) * 10.0f);
            hmap[i][j] = h;
        }
    }

    for (int i2 = i; i < u / 2 - 1; i++, i2++)
        for (int j = 0; j < v; j++)
        {
            float x = ((float)i2 / (float)u) - 0.25f;
            float y = ((float)j / (float)v) - 0.25f;
            float xx = ((float)i2 / (float)u) - 0.5f;
            float yy = ((float)j / (float)v) - 0.5f;
            float h = xx * yy * xx * yy - 0.5f / (1.0f + (x * x + y * y) * 10.0f);
            hmap[i][j] = h;
        }
    for (int i3 = 0; i < u; i++, i3++)
        for (int j = 0; j < v; j++)
        {
            float x = ((float)i3 / (float)u) - 0.25f;
            float y = ((float)j / (float)v) - 0.25f;
            float xx = ((float)i3 / (float)u) - 0.5f;
            float yy = ((float)j / (float)v) - 0.5f;
            float h = xx * yy * xx * yy + 0.3f / (0.5f + (x * x + y * y) * 20.0f);
            hmap[i][j] = h;
        }
}
//--------------------------------------------------------------------------------------
...
{
	// Вершинный буфер
    SimpleVertex vertices[u * v + 9];

	// Индексный буфер
	WORD indices[IndicesCount + 42];
    ...
    for (int i = 0; i < u; i++)
        for (int j = 0; j < v; j++)
        {
            ...
            //int val = rand() % 3;
            vertices[j * u + i].Color = XMFLOAT4(colors[rand() % 3][0], colors[rand() % 3][1], colors[rand() % 3][2], 1.0f);
            vertices[j * u + i].Normal = XMFLOAT3(0, 0, 0);
        }

    SimpleVertex verticesObj[] =
    {
        ...
        { XMFLOAT3(-3.0f, 3.0f, 0.0f), XMFLOAT4(1.0f, 0.0f, 0.0f, 1.0f) }, // 4
        { XMFLOAT3(3.0f, 5.0f, 0.0f), XMFLOAT4(1.0f, 0.0f, 0.0f, 1.0f) }, // 5
        { XMFLOAT3(0.0f, 4.0f, -3.0f), XMFLOAT4(1.0f, 0.0f, 0.0f, 1.0f) }, // 6
        { XMFLOAT3(0.0f, 2.0f, 3.0f), XMFLOAT4(1.0f, 0.0f, 0.0f, 1.0f) }, // 7

        { XMFLOAT3(0.0f, 2.0f, 0.0f), XMFLOAT4(1.0f, 0.0f, 0.0f, 1.0f) }, // 8
    };

    for (int i = 0; i < 9; i++)
    {
        vertices[u * v + i].Pos = verticesObj[i].Pos;
        vertices[u * v + i].Color = verticesObj[i].Color;
        vertices[u * v + i].Normal = verticesObj[i].Normal;
    }
    ...
    for (int i = 0; i < 42; i++)
    {
        indices[IndicesCount + i] = indicesObj[i] + u * v;
    }
 
    D3D11_BUFFER_DESC bd;
	ZeroMemory(&bd,sizeof(bd));
    bd.Usage = D3D11_USAGE_DEFAULT;
    bd.ByteWidth = sizeof( SimpleVertex ) * (u*v + 9);   
    ...
    bd.Usage = D3D11_USAGE_DEFAULT;
    bd.ByteWidth = sizeof( WORD ) * (IndicesCount + 42);       
...
void Render()
{
    ...
    g_World1 = XMMatrixTranslation(0, 2, 0) * XMMatrixScaling(0.5f, 0.5f, 0.5f);
    ConstantBuffer cb1;
    cb1.mWorld = XMMatrixTranspose(g_World1);
    cb1.mView = XMMatrixTranspose(g_View);
    cb1.mProjection = XMMatrixTranspose(g_Projection);
    g_pImmediateContext->UpdateSubresource(g_pConstantBuffer, 0, NULL, &cb1, 0, 0);
    g_pImmediateContext->DrawIndexed(42, IndicesCount, 0);

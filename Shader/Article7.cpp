...
HRESULT InitWindow( HINSTANCE hInstance, int nCmdShow )
{
    ...
    g_hWnd = CreateWindow( L"Article7", L"Урок 7: Текстуры", WS_OVERLAPPEDWINDOW,
...
HRESULT InitGeometry()
{
	...
	HRESULT hr;
    hr = CompileShaderFromFile( L"Article7.fx", "VS", "vs_4_0", &pVSBlob );
    if( FAILED( hr ) )
    {
        MessageBox( NULL,
    ...
    hr = CompileShaderFromFile( L"Article7.fx", "PS", "ps_4_0", &pPSBlob );
    if( FAILED( hr ) )
    {
        MessageBox( NULL,
                    L"The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", L"Error", MB_OK );
    ...
    // Загрузка текстуры
    /*hr = D3DX11CreateShaderResourceViewFromFile( g_pd3dDevice, L"images.dds", NULL, NULL, &g_pTextureRV, NULL );
    if( FAILED( hr ) )
        return hr;*/
...
void Render()
{
	...
	XMVECTOR Eye = XMVectorSet( 2.5f*sin(0.00025f)*radius, 1.0f, -2.0f+cos(0.00025f)*radius*2.0f, 0.0f );
	...
	for (int i=0; i<3; i++)
	{
        cb.mWorld = XMMatrixTranspose(XMMatrixTranslation(0,0,0));//((float)i-1.0f)*6.0f,-3.0f,0) * XMMatrixRotationZ(orbit * 2.0f));
	    g_pImmediateContext->UpdateSubresource( g_pConstantBuffer, 0, NULL, &cb, 0, 0 );
	    g_pImmediateContext->VSSetConstantBuffers( 0, 1, &g_pConstantBuffer );
	    g_pImmediateContext->DrawIndexed( 42, 0, 0 );       // 42 вершин образуют 14 полигонов, по три вершины на полигон
	}
